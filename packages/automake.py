class Automake (GnuPackage):
    ...
    def arch_build(self, arch):
        if arch == 'darwin-universal':
            self.local_ld_flags = ['-arch i386', '-arch x86_64', '-arch arm64']
            self.local_gcc_flags = ['-arch i386', '-arch x86_64', '-arch arm64']
        elif arch == 'darwin-universal64':
            self.local_ld_flags = ['-arch x86_64', '-arch arm64']
            self.local_gcc_flags = ['-arch x86_64', '-arch arm64']
        elif arch == 'darwin-32':
            ...
            self.local_ld_flags = ['-arch arm64 -m64']
            self.local_gcc_flags = ['-arch arm64 -m64']
Automake()
